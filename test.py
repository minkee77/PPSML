...
# ============================ Data & Networks =====================================
from dataset.datasets_csv import Imagefolder_csv, dataset_tiered
...
model_trained = './results/PPSML_miniImageNet_Conv64F_ImgtoClass_5Way_5Shot_K3_SK3/model_best.pth.tar'
...
parser.add_argument('--outf', default='./results/PPSML')
...
parser.add_argument('--metric', default='ImgtoClass', help='ImgtoClass|Prototype')
...
parser.add_argument('--unlabel_num', type=int, default=30, help='the number of unlabeled samples')
parser.add_argument('--neighbor_k', type=int, default=3, help='the number of k-nearest neighbors')
parser.add_argument('--semi_neighbor_k', type=int, default=5, help='the number of k-nearest neighbors used in semi-supervised setting')
parser.add_argument('--semi_ratio', type=int, default=2, help='the ratio used in semi-supervised setting')
...
def validate(val_loader, model, criterion, epoch_index, F_txt):
	...
	for episode_index, (query_images, query_targets, support_images, support_targets, unlabel_images, unlabel_targets) in enumerate(val_loader):
		...
		unlabel_images = torch.cat(unlabel_images, 0)
		input_var3 = unlabel_images.cuda()
...
# ======================================== Settings of path ============================================
# saving path
opt.outf = opt.outf+'_'+opt.data_name+'_'+str(opt.basemodel)+'_'+str(opt.metric)+'_'+str(opt.way_num)+'Way_'+str(opt.shot_num)+'Shot'+'_K'+str(opt.neighbor_k)+'_SK'+str(opt.semi_neighbor_k)
...
for r in range(repeat_num):
	...
	if opt.data_name == 'tieredImageNet':
		testset = dataset_tiered(
			data_dir=opt.dataset_dir, mode=opt.mode, image_size=opt.imageSize, transform=ImgTransform,
			episode_num=opt.episode_test_num, way_num=opt.way_num, shot_num=opt.shot_num, query_num=opt.query_num, unlabel_num=opt.unlabel_num
		)
	else:
